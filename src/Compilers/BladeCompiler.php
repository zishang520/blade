...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php if (\luoyy\Blade\Support\Blade::check('{$name}', {$expression})): ?>"
                : "<?php if (\luoyy\Blade\Support\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else' . $name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\luoyy\Blade\Support\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\luoyy\Blade\Support\Blade::check('{$name}')): ?>";
        });
    ...
    public function aliasInclude($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            ...
            return "<?php echo \$__env->make('{$path}', {$expression}, \luoyy\Blade\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>";
        });
    }
